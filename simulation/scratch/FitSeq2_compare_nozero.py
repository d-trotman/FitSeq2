...
def compare_frequencies(chemostat_sim, discrete_sim, 
    ...
    with proper handling of filtered data
    """
    # Load original data
    ...
    # Extract read columns to identify which strains were kept in the filtered dataset
    read_columns = [f'reads_t{t}' for t in range(num_timepoints)]
    
    # Create filter mask - strains with non-zero reads across all timepoints
    nonzero_mask = (discrete_data[read_columns] > 0).all(axis=1)
    
    # Filter the original discrete data to match what was used in FitSeq2
    discrete_data_filtered = discrete_data[nonzero_mask].reset_index(drop=True)
    
    # Verify sizes match
    print(f"Original discrete data: {len(discrete_data)} strains")
    print(f"Filtered discrete data: {len(discrete_data_filtered)} strains")
    print(f"Discrete FitSeq results: {len(discrete_fitseq)} strains")
    
    if len(discrete_data_filtered) != len(discrete_fitseq):
        print("WARNING: Filtered data size doesn't match FitSeq results size!")
    ...
    for idx, t in enumerate(range(1, num_timepoints)):
        ...
        # Use filtered discrete data that matches FitSeq input
        discrete_orig_freq = discrete_data_filtered[f'frequency_t{t}'].values
        
        # Calculate FitSeq2 frequencies
    ...
    plt.tight_layout()
    plt.savefig('frequency_comparison_log_filter.png', dpi=300)
    plt.close()
...
def main():
    ...
    discrete_fitseq_reads = 'discrete_filtered_results_FitSeq2_Result_Read_Number_Estimated.csv'
